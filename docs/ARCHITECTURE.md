...
The second adapter is `amp`, a skeleton behind the same phase contract. It records Amp provider request prompts/schemas and audit metadata for every phase while keeping execution controlled by the platform. Its provider requests include readiness/config metadata from `src/core/amp-runtime.mjs`, including command availability, live-invocation opt-in state, blockers/warnings, and secret-safe API-key presence. Live provider phases currently implemented are `interpret_requirement`, `create_task_breakdown`, and `create_implementation_plan`; they run only when `AGENT_SDLC_AMP_LIVE=true`, `AGENT_SDLC_AMP_ALLOW_NETWORK=true`, and readiness passes. Live interpret/planning are read-only/no-write, send prompts on stdin to the configured command, validate JSON schemas, and persist raw/parsed provider output artifacts. The execution phase still uses the control plane's explicit local config edit (`--target-file`, `--set-key`, `--set-value`) so Amp cannot perform uncontrolled SDK/CLI writes. The next step is to add live Amp review behind the same readiness checks and schema validation.
